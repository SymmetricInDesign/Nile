...
product1 = Product.find_or_create_by(category_id: cat1.id, name: "Modern Mid-Century Side Chair with Natural Wood Legs for Kitchen, Living Dining Room, White", details: "<ul>
...
product2 = Product.find_or_create_by(category_id: cat2.id, name: "40-inch 1080p Smart LED Roku TV", details: "Easy Voice Control: Works with Amazon Alexa or Google Assistant to help you find movie titles, launch or change channels, even switch inputs, using just your voice. Also available through the Roku mobile app
    ...
    Inputs: 3 HDMI, 1 USB, RF, Composite, Headphone Jack, optical audio out", price: 155.99)
...
product3 = Product.find_or_create_by(category_id: cat2.id, name: "27\" FHD Flat Monitor with Super-Slim Design", details: "<ul>
...
product4 = Product.find_or_create_by(category_id: cat2.id, name: "Laptop PC 11.6\" Intel N4000 Quad Core 4GB DDR4 SDRAM 32GB eMMC", details: "<ul>
...
product5 = Product.find_or_create_by(category_id: cat1.id, name: "Folding Desk No Assembly Required, 40\" Writing Computer Desk", details: "<ul>
...
product6 = Product.find_or_create_by(category_id: cat1.id, name: "Convertible Sectional Sofa for Small Space L-Shaped Couch with Modern Linen Fabric", details: 
'<ul>
